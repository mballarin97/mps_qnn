...
if __name__ == '__main__': 
    ...
    range_sigma = np.logspace(-5, 3, 15)
    for i in range_sigma:
        mean = np.pi/2.
        sigma = i * np.pi
        #VALORE DI SIGMA APPROSSIMATO PER IL SUO NOME
        sigma_round = round(i, -int(floor(log10(abs(i)))) )
        sigma_pi = format_e(Decimal(sigma_round))
        ...
        #ciclo for che mi analizza i circuiti con un numero di qubit preso dal range e sempre diverso
            ...
            for alternate in alternate_possibilities: 
                ...
                if(len(ent_means) < 10):    
                    ...
                    for indx in range:
                        ...
                        marker="o", c=cmap[indx], label=f'L={indx+1}')
                else:
                    range = np.arange(0, 9, dtype=int)
                    for indx in range:
                        plt.plot(x, ent_means[indx, :], ls="-",
                        marker="o", c=cmap[indx], label=f'L={indx+1}')

                    range2 = np.arange(0, len(ent_means)-9, dtype=int)
                    print(range2)
                    for indx2 in range2:
                        plt.plot(x, ent_means[indx2 + 9, :], ls="-",
                        marker="o", c=cmap[indx2], label=f'L={indx2+10}')
