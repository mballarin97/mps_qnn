...
def _run_mps(qc, max_bond_dim=1024):
    """
...
def _run_circuit(qc, parameters, max_bond_dim=1024):
    """
    ...
    results = _run_mps(qc, max_bond_dim=max_bond_dim)
    return results

def _mps_simulation(qc, random_params, max_bond_dim=1024):
    """"
...
def entanglement_characterization(ansatz = None, backend = 'Aer', get_statevector = False):
    ...
    if backend == 'MPS':    
        ent_means, ent_std = _mps_simulation(ansatz, random_params, max_bond_dim)
